...
class ConductorManager(periodic_task.PeriodicTasks):
    ...
    def change_node_power_state(self, context, node_id, new_state):
        ...
        with task_manager.acquire(context, node_id, shared=False) as task:
            ...
            task.node.save()
            task.set_spawn_error_hook(_power_state_error_handler,
    ...
    def get_driver_vendor_passthru_methods(self, context, driver_name):
        ...
        return _get_vendor_passthru_metadata(driver.vendor.driver_routes)
    ...
    def do_node_deploy(self, context, node_id, rebuild=False):
        ...
        with task_manager.acquire(context, node_id, shared=False) as task:
            ...
            try:
                ...
                task.set_spawn_error_hook(_provisioning_error_handler,
                                          node, previous_prov_state,
                                          previous_tgt_provision_state)
                task.spawn_after(self._spawn_worker,
                                 _do_node_deploy, task, self.conductor.id)
        ...
        for (node_id, node_uuid, driver) in node_list:
            try:
                if not self._mapped_to_this_conductor(node_uuid, driver):
                    continue
                # NOTE(deva): This section is redundant, but unit tests want it
                node = objects.Node.get_by_id(context, node_id)
                if ((node.provision_state == states.DEPLOYWAIT) or
                        node.maintenance or node.reservation):
                    continue
                ...
                with task_manager.acquire(context, node_id) as task:
                    if ((task.node.provision_state == states.DEPLOYWAIT) or
                        ...
                        continue
                    result = _do_sync_power_state(
                            task, self.power_state_sync_count[node_uuid])
                    if result:
                        self.power_state_sync_count[node_uuid] = result
                    else:
...
def _get_vendor_passthru_metadata(route_dict):
    d = {}
...
def _power_state_error_handler(e, node, power_state):
...
def _provisioning_error_handler(e, node, provision_state,
...
def _do_node_deploy(task, conductor_id):
...
def _handle_sync_power_state_max_retries_exceeded(task,
                                                  actual_power_state):
    node = task.node
...
def _do_sync_power_state(task, count):
    """Sync the power state and increment the counter upon failure."""
    node = task.node
    power_state = None
    ...
    try:
        ...
        if (count >= CONF.conductor.power_state_sync_max_retries):
            _handle_sync_power_state_max_retries_exceeded(task, power_state)
            return count
        else:
            return count
    ...
    try:
        ...
        LOG.error(_LE("Failed to change power state of node %(node)s "
                      "to '%(state)s'."), {'node': node.uuid,
                                           'state': node.power_state})
        attempts_left = CONF.conductor.power_state_sync_max_retries - count
